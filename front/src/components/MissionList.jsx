...
import { getRole } from '../services/authService'; // 🔹 ajouté
...
function MissionList({ onEdit, refresh }) {
    ...
    const role = getRole(); // 🔹 ajouté

    const fetchData = async () => {
        try {
            console.log('Token envoyé :', localStorage.getItem('token')); // Débogage : Vérifie si le token est présent
            console.log('Rôle :', role); // Débogage : Vérifie le rôle
            const res = await getMissions();
            setMissions(res.data);
        } catch (error) {
            console.error('Erreur API :', error.response?.data); // Débogage : Affiche le détail de l'erreur du serveur
